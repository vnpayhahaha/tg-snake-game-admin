  ...
  setup() {
    ...
    const toolbarList = computed(() => {
      ...
      return toolbarHook.toolbars.value.map((item: MineToolbar) => {
        const savedItem = toolbarSettings.find(
          (setting: MineToolbar) => setting.name === item.name
        );
        return {
          ...item,
        ...
        [
          {
            ...
            label: useTrans("mineAdmin.settings.colorModes.light") as string,
            value: "light",
          },
          {
            icon: "material-symbols:dark-mode-outline",
            label: useTrans("mineAdmin.settings.colorModes.dark") as string,
            value: "dark",
          },
          {
            icon: "lets-icons:color-mode-light",
            label: useTrans("mineAdmin.settings.colorModes.system") as string,
            value: "autoMode",
          },
        ],
          ...
          {colorList.map((color: string) => (
            ...
            >
              {settingStore.getSettings("app").primaryColor === color && (
                <ma-svg-icon
                  class="icon"
                  name="i-material-symbols:check-small"
                />
              )}
            </li>
          ))}
        ...
        <div class="mine-layout-setting">
          <m-tooltip text={useTrans("mineAdmin.settings.layouts.classic")}>
            <div
              class={{
                classic: true,
                "mine-selected-layout": settingStore.isClassicLayout(),
              }}
              onClick={() =>
                (settingStore.getSettings("app").layout = "classic")
              }
            >
          ...
          <m-tooltip text={useTrans("mineAdmin.settings.layouts.columns")}>
            <div
              class={{
                columns: true,
                "mine-selected-layout": settingStore.isColumnsLayout(),
              }}
              onClick={() =>
                (settingStore.getSettings("app").layout = "columns")
              }
            >
          ...
          <m-tooltip text={useTrans("mineAdmin.settings.layouts.mixed")}>
            <div
              class={{
                mixed: true,
                "mine-selected-layout": settingStore.isMixedLayout(),
              }}
    ...
    const settingsRender = (
      name: SystemSettings.settingType,
      options: { label: string; value: string; change?: (e: boolean) => void }[]
    ) => {
            ...
            <div class="mine-setting-description">
              ...
              <div class="desc-value">
                <m-switch
                  ...
                  onChange={(e: boolean) => item?.change?.(e)}
                />
              </div>
            </div>
          ))}
      ...
      return (
        <>
          {settingsRender("tabbar", [
            {
              label: useTrans("mineAdmin.settings.tabbars.enable") as string,
              value: "enable",
            },
          ])}
          <div class="mine-setting-description">
            <div class="desc-label">
              {useTrans("mineAdmin.settings.tabbars.mode")}
            </div>
            <div class="desc-value">
              {tabsRender(
                "tabMode",
                [
                  {
                    label: useTrans(
                      "mineAdmin.settings.tabbars.modeDefault"
                    ) as string,
                    value: "rectangle",
                  },
                  {
                    label: useTrans(
                      "mineAdmin.settings.tabbars.modeCard"
                    ) as string,
                    value: "card",
                  },
                ],
                (v: string) => (settingStore.getSettings("tabbar").mode = v),
                "h-6 text-[12px]"
              )}
            </div>
          </div>
    ...
    const copyrightRender = () => {
      ...
      const options = [
        {
          label: useTrans("mineAdmin.settings.copyRights.date") as string,
          value: "dates",
        },
        {
          label: useTrans("mineAdmin.settings.copyRights.company") as string,
          value: "company",
        },
        {
          label: useTrans("mineAdmin.settings.copyRights.website") as string,
          value: "website",
        },
        {
          label: useTrans(
            "mineAdmin.settings.copyRights.putOnRecord"
          ) as string,
          value: "putOnRecord",
        },
      ];
      return (
        <>
          {settingsRender("copyright", [
            {
              label: useTrans("mineAdmin.settings.copyRights.enable") as string,
              value: "enable",
            },
          ])}
          {options.map((item: any) => (
          ...
          v-slots={{
            ...
            default: () => (
              <div class="pb-10">
                ...
                <div class="mx-auto mt-3 w-[70%]">{colorModeSettings()}</div>
                {keys.value.colorMode !== "dark" &&
                  settingsRender("app", [
                    {
                ...
                {divider(
                  useTrans("mineAdmin.settings.primaryColorSetting") as string
                )}
                <ul class="mine-setting-color-list">{colorListRender()}</ul>

                {divider(
                  useTrans("mineAdmin.settings.layoutSetting") as string
                )}
                {layoutsRender()}

                {divider(
                  useTrans("mineAdmin.settings.mainAsideSetting") as string
                )}
                {settingsRender("mainAside", [
                  {
                    label: useTrans(
                      "mineAdmin.settings.mainAsides.mainMenuTitle"
                    ) as string,
                    value: "showTitle",
                  },
                  {
                    label: useTrans(
                      "mineAdmin.settings.mainAsides.autoToFirstMenu"
                    ) as string,
                    value: "enableOpenFirstRoute",
                  },
                ])}

                {divider(
                  useTrans("mineAdmin.settings.tabBarSettings") as string
                )}
                {tabSettingRender()}

                {divider(
                  useTrans("mineAdmin.settings.toolBarSettings") as string
                )}
                {toolbarHook.state && toolbarRender()}

                {divider(
                  useTrans("mineAdmin.settings.copyRightSettings") as string
                )}
                {copyrightRender()}

                {divider(
                  useTrans("mineAdmin.settings.otherSettings") as string
                )}
                {settingsRender("app", [
                  {
                    label: useTrans(
                      "mineAdmin.settings.enableBreadcrumb"
                    ) as string,
                    value: "showBreadcrumb",
                  },
                  {
                    label: useTrans(
                      "mineAdmin.settings.enableWatermark"
                    ) as string,
                    value: "enableWatermark",
                  },
                ])}
                <div class="mine-setting-description">
                  <div class="desc-label !w-6/12">
                    {useTrans("mineAdmin.settings.watermarkText")}
                  </div>
                  <div class="desc-value !w-6/12">
                    <m-input
                      class="mb-1 !h-6 !py-1"
                      ...
                      onInput={(v: InputEvent) => {
                        ...
                        settingStore.getSettings("app").watermarkText =
            ...
            footer: () => (
              <m-button
                ...
                onClick={() => {
                  ...
                  Message.success(t("mineAdmin.common.saveSuccess"), {
                    zIndex: 9999,
                  });
                }}
              >
