...
import { deleteByIds, page } from '~/tenant/api/TenantApiInterface.ts'
...
defineOptions({ name: 'tenant:tenant_api_interface' })
...
const options = ref<MaProTableOptions>({
  ...
  header: {
    mainTitle: () => t('tenant_api_interface.index'),
  },
  // 表格参数
  tableOptions: {
    on: {
...
<template>
  <div class="mine-layout pt-3">
    <MaProTable ref="proTableRef" :options="options" :schema="schema">
      <template #actions>
        <el-button
          v-auth="['tenant:tenant_api_interface:save']"
          type="primary"
          @click="() => {
      ...
      <template #toolbarLeft>
        <el-button
          v-auth="['tenant:tenant_api_interface:delete']"
          type="danger"
          plain
