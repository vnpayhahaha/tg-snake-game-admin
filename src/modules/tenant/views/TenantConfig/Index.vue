...
import { deleteByIds, page } from '~/tenant/api/TenantConfig.ts'
...
defineOptions({ name: 'tenant:tenant_config' })
...
const options = ref<MaProTableOptions>({
  ...
  header: {
    mainTitle: () => t('tenantConfig.index'),
  },
  // 表格参数
  tableOptions: {
    on: {
...
<template>
  <div class="mine-layout pt-3">
    <MaProTable ref="proTableRef" :options="options" :schema="schema">
      <template #actions>
        <el-button
          v-auth="['tenant:tenant_config:save']"
          type="primary"
          @click="() => {
      ...
      <template #toolbarLeft>
        <el-button
          v-auth="['tenant:tenant_config:delete']"
          type="danger"
          plain
