  ...
  return [
    ...
    // 投注金额
    {
      ...
      cellRenderTo: {
        ...
        props: {
          type: 'text',
          format: (row: GameGroupConfigVo) => `${row.bet_amount} TRX`,
        },
      },
    },
    // 平台手续费比例
    {
      ...
      cellRenderTo: {
        ...
        props: {
          type: 'text',
          format: (row: GameGroupConfigVo) => `${(row.platform_fee_rate * 100).toFixed(2)}%`,
        },
      },
    },
    // 钱包变更状态
    {
      ...
      cellRenderTo: {
        ...
        props: {
          type: 'tag',
          tagType: (row: GameGroupConfigVo) => {
            return row.wallet_change_status === 1 ? 'success' : 'warning'
          },
          format: (row: GameGroupConfigVo) => {
            return row.wallet_change_status === 1 ? t('gameGroupConfig.walletChangeNormal') : t('gameGroupConfig.walletChangeChanging')
          },
        },
      },
    },
    ...
    // 操作列
    {
      ...
      operationConfigure: {
        ...
        actions: [
          {
            name: 'edit',
            show: (row: GameGroupConfigVo) => showBtn('tg_game:config:update', row),
            text: () => t('crud.edit'),
            on: {
              click: (row: GameGroupConfigVo, proxy: MaProTableExpose) => {
          ...
          {
            name: 'walletChange',
            show: (row: GameGroupConfigVo) => showBtn('tg_game:config:startWalletChange', row) && row.wallet_change_status === 1,
          ...
          },
          {
            ...
            show: (row: GameGroupConfigVo) => showBtn('tg_game:config:cancelWalletChange', row) && row.wallet_change_status === 2,
            text: () => t('gameGroupConfig.cancelWalletChange'),
            type: 'warning',
            on: {
              click: async (row: GameGroupConfigVo, proxy: MaProTableExpose) => {
          ...
          },
          {
            ...
            show: (row: GameGroupConfigVo) => showBtn('tg_game:config:completeWalletChange', row) && row.wallet_change_status === 2,
          ...
          {
            name: 'delete',
            show: (row: GameGroupConfigVo) => showBtn('tg_game:config:delete', row),
            text: () => t('crud.delete'),
            type: 'danger',
            on: {
              click: async (row: GameGroupConfigVo, proxy: MaProTableExpose) => {
